...
![[Pasted image 20220916111939.png]]
...
-   `taskQueue`是一个小顶堆数组, 关于堆排序的详细解释, 可以查看[React 算法之堆排序](https://7kms.github.io/react-illustration-series/algorithm/heapsort).
-   源码中除了`taskQueue`队列之外还有一个`timerQueue`队列. 这个队列是预留给延时任务使用的, 在 [react@17.0.2](mailto:react@17.0.2) 版本里面, 从源码中的引用来看, 算一个保留功能, 没有用到.
...
`flushWork`中调用了`workLoop`. 队列消费的主要逻辑是在`workLoop`函数中, 
...
在reconciler 运作流程中总结的 4 个阶段中, `注册调度任务`属于第 2 个阶段, 核心逻辑位于`ensureRootIsScheduled`函数中. 现在我们已经理解了`调度原理`, 再次分析`ensureRootIsScheduled`
