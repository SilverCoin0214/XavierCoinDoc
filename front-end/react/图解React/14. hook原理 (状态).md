...
`创建hook`之后返回值`[hook.memoizedState, dispatch]`中的`dispatch`实际上会调用`reducer`函数.
...
![[Pasted image 20220921101645.png]]
...
从调用`scheduleUpdateOnFiber`开始, 进入了`react-reconciler`包, 

注意: 本示例中虽然同时执行了 3 次 dispatch, 会请求 3 次调度, 由于调度中心的[节流优化](https://7kms.github.io/react-illustration-series/main/hook-state/scheduler##throttle-debounce), 最后只会执行一次渲染
...
![[Pasted image 20220921105606.png]]
...
![[Pasted image 20220921110336.png]]

3. 状态计算
	1.  `update`优先级不够: 加入到 baseQueue 中, 等待下一次 render   
	2.  `update`优先级足够: 状态合并
	3.  更新属性
![[Pasted image 20220921110408.png]]
...
![[Pasted image 20220921111823.png]]

链表拼接:

-   和同步更新时一致, 直接把`queue.pending`拼接到`current.baseQueue`

![[Pasted image 20220921111853.png]]
...
![[Pasted image 20220921112018.png]]
