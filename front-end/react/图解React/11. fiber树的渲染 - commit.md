...
在`commitRoot`中同时使用到了`渲染优先级`和`调度优先级`,
`commitRootImpl`函数:
```js
// ... 省略部分无关代码
...
![[Pasted image 20220920162246.png]]
...
![[Pasted image 20220920162548.png]]
...
2.  处理`Passive`标记
`Passive`标记只会在使用了`hook`对象的`function`类型的节点上存在,
通过`scheduleCallback`单独注册了一个调度任务`task`, 等待调度中心`scheduler`处理.
...
![[Pasted image 20220920164940.png]]
